...
line crosspoint(circle c, line l)
{
    point h = proj(l, c.o);
    double d2 = norm(h - c.o);
    point v = sqrt(max(0.0L, c.r * c.r - d2)) * (l.b - l.a) / abs(l.b - l.a);
    return line(h - v, h + v);
}
...
void solve()
{
    ...
    if (abs(vh - abs(vm)) < eps) {
        ...
        double ans = inf;
        if (real(cand) != inf and dot(vm, cand - monkey) > 0.0L) {
    ...
    double ans = inf;
    if (dot(vm, cand.a - monkey) > 0.0L) {
        ans = min(ans, abs(cand.a - monkey) / abs(vm));
    }
    if (dot(vm, cand.b - monkey) > 0.0L) {
